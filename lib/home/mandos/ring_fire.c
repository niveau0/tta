...
create_object()
{
    set_name("ring");
    set_pname("rings");
    set_short("ring with a dark red stone");
    set_pshort("rings with a dark red stone");
    set_long("A ring made of enchanted iron with a dark red stone in it. "+
...
object_worn(object tp, object ob)
{
    if (!tp || ob != TO())
	return;
    tp->add_tmp_skill(SK_RESIST_HEAT, 30);
}
