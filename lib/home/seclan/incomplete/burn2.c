...
check_fire()
{
	...
	if(burning <= 0)
	{
	    // object wasn't burning until now
...
distribute_heat()
{
    ...
    {
        neighb_objects = env->inventory(0);
        env->add_temperature(temperature-(env->query_temperature()));
        heat_drain = 0;
	...
	heat_drain = heat_drain/(drain_objects+2); // +1 for itself and +1 for environment
	// set the energy for neighbours/environment/object
        for (i = drain_objects; i >= 0; i--)
        {
	    neighb_objects[i]->set_temperature(heat_drain);
        }
	env->set_temperature(heat_drain);
