...
create_room()
{
    ...
    climate = 1;
    height = 100;
    room_coords =({-9999999, -9999999, 9999990 });
...
void
do_move(int movement, int next_stop)
{
    ...
    if(next_stop)
	call_out("do_move", 3, movement, next_stop);
    else
	for(i=0;i<sizeof(elevator_door);i++)
...
{
    int movement, i, next_z, tmp_z;

    remove_call_out("do_move");
    
    for(i=0;i<sizeof(elevator_door);i++)
    {
	tmp_z = elevator_door[i]->query_next_room_level(room_coords[2],direction);
	(elevator_door[i]->query_elevator_door())->do_close();
	if((tmp_z<next_z) || (next_z == 0))
	    next_z = tmp_z;
    }
    next_z = room_coords[2] - next_z;
	...
	call_out("do_move", 3, movement, next_z);
    }
