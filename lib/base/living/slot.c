...
private mapping wield,       /* all wielded items */
                worn,        /* all worn items */
                hold,        /* all held items */
                protection,  /* all protected slots */
...
slot_reset()
{
    ...
    protection  = ([]); /* used to speed up collection */
    slot_sizes  = ([]);
    slot_descs  = ([]);
    slot_types  = ([]);
    wounds      = ([]);
 ...
 * Description:   find a specified location within a list of locations
 * Arguments:     locs - list of locations to search
 *                pref_slot - prefered slot that must appear within the list
 * Returns:       list of modified locations including the prefered slot if
 *                found, else 0
 */
nomask int *
find_location(int *locs, int pref_slot)
{
    int i;

    for (i=sizeof(locs); i--;)
    {
	if (pref_slot == locs[i]) /* exact match */
	    return locs;
	if (SLOT_TYPE(pref_slot) == locs[i]) /* matching non-special slot */
	{
	    locs[i] = pref_slot;
	    return locs;
	}
...
wear_tool(object tool, int pref_slot)
{
	...
	if (!(locs = find_location(locs, pref_slot)))
	    return -1;
    }
	...
	if (!(left = left_slots[loc]))
	{
	    if (!slot_types[loc])
		return -1;
	    left = left_slots[loc] = slot_types[loc][..];
	}
	if (!(sz2 = sizeof(left)))
	    return -1;
	for (j=0; j<sz2; j++)
	...
	if (!worn[loc])
	    ...
	    worn[loc] = ({ tool }) + worn[loc]; /* don't change order! */
	desc[i] = slot_descs[loc];
    }
    locs = tool->query_protection();
    for (i=sizeof(locs); i--;)
    {
	loc = locs[i];
	if (loc % 100 <= 0) 
	{
	    /* if there is no left, right, whatever - specification
	     * then try to match a protection-slot with a used location-slot
	     */
	    for (j=sizeof(used); j--;)
		if (used[j] % 100 == loc)
		{
	    ...
	    if (j < 0)
	    {
		/* no match, so try to find unprotected slot (by this tool)
		 * matching the current slottype
		 */
		left = slot_types[loc];
		for (j=sizeof(left); j--;)
		    if (!protection[left[j]] ||
			!IN_ARRAY(tool, protection[left[j]]))
		    {
			loc = left[j];
			break;
		    }
		if (j < 0) /* found no more slots to protect */
		    continue;
	    }
	}
	if (!protection[loc])
...
remove_tool(object tool, int flag) 
{
    ...
    {
	slots = map_indices(worn); /* all slots that contain armor */
	for (i=sz=sizeof(slots); i--;)
	{
	    curr = worn[slots[i]];
	    if (!sizeof(curr) || curr[0] == tool)
		continue;
	    if (!IN_ARRAY(tool, curr))
	    {
		/* tool not in this slot, remove from searchlist */
		slots[i] = 0; 
		continue;
	    }
	...
	for (i=sz; i--;)
	    if (slots[i])
		worn[slots[i]] -= ({ tool });
	slots = map_indices(protection); /* all protected slots */
	for (i=sizeof(slots); i--;)
	    protection[slots[i]] -= ({ tool });
 ...
 * Description:   Returns the protecting tools for the given slot
 * Arguments:	  slot - The toolslot to query
 * Return:        object array
 */
nomask object *
query_protection(int slot)
{
    if (!protection[slot])
	return ({ });
    protection[slot] -= ({ 0 });
    return protection[slot];
}
...
/*
 * Function name: filter_visible_armor
 * Description:   filters all objects which cannot be seen, because all
 *                their slots are covered by clothes in a layer over them
 *                (see slots.h)
 * Returns:       object array, the worn objects that can be seen
 */
nomask object *
filter_visible_armor()
{
    int    i, *slots;
    object *obj, *seenobj;

    seenobj = ({ });
    for (i=sizeof(slots = query_slots()); i--;)
    {
	if (sizeof(obj = worn[slots[i]]))
	    seenobj |= ({ obj[0] });
    }
    return seenobj;
}
...
show_inventory(object for_ob)
{
    ...
    if (sizeof(o = filter_visible_armor()) &&
	(str = COMPOSITE_UNSEEN(o, for_ob)))
    {
	if (for_ob == this_object())
...
set_wound(int slot, int flag)
{
	...
	if (SLOT_TYPE(slot) == SLT_EYE && sizeof(query_slot_type(SLT_EYE)) > 0)
	    remove_effect(BLIND_EFFECT, EFF_LOST_EYES);
	return;
    }
    if (flag == SLOT_LOST)
    {
    ...
    if (SLOT_TYPE(slot) == SLT_EYE)
    {
	/* no eyes cause blindness */
	if (!sizeof(query_slot_type(SLT_EYE)))
	    add_effect(BLIND_EFFECT, EFF_LOST_EYES, 1);
    }
}
