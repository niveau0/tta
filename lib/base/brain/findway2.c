 ...
 *                or else the number of waypoints.
 */
private int
brain_find_way(object ob)
{
    ...
    if (!(sz = sizeof(blockers = room->query_event(E_BLOCK))))
    {
	togosz=0;
	approach_obj=ob;
	target_field=({ });
 ...
 * Description:   actually approach coordinates of an object.
 *                Possibly call brain_find_way() to compute a new path.
 *                if something blocks, try next directions.
 * Arguments:     target - object to approach
...
brain_approach(object target)
{
    ...
    if ((env=environment(me=this_object())) == environment(target))
    	return -1;
	...
	{
	    direct_approach(0, approach_obj);
	    approach_obj=0;
