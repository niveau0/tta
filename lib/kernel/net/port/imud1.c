...
reopen()
{
    _DRIVER->message("Trying to reopen port " + PORT_IMUD1_UDP +
			" (intermud1).\n");
    open_port("udp", PORT_IMUD1_UDP);
}
...
open(int p)
{
    call_out("message", 0, "Port " + PORT_IMUD1_UDP +
	     " (intermud1) successfully opened.\n");
}
...
{
    if (object_name(previous_object()) != I1_DEMON)
	error("Illegal call to send_udp");
/*    debug("manwe", "imud1 currently disabled\n");*/
...
receive_datagram(string data, string ipnum, int p)
{
	...
	return 0;
    set_this_player(0);
    I1_DEMON->receive_udp(ipnum, data, p);
    return 0;
}
