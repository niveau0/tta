...
door_move(object ob, object from, object to)
{
    ...
    if (to->query_room_file() != BREE_PATH+"/Prancing_Pony/private_dining")
    {
	set_direction_desc("leading into a small room");
	set_connection(BREE_PATH+"/Prancing_Pony/private_dining", 1);
    }
    else
    {
