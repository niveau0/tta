...
door_move(object ob, object from, object to)
{
    ...
    if (to->query_room_file() != BREE_PATH+"/Prancing_Pony/dining_room")
    {
	set_direction_desc("leading into a large room");
	set_connection(BREE_PATH+"/Prancing_Pony/dining_room", 1);
    }
    else
    {
