...
door_move(object ob, object from, object to)
{
    ...
    if (to->query_room_file() != BREE_PATH+"/Prancing_Pony/archway")
    {
	set_direction_desc("leading outside to an archway");
	set_connection(BREE_PATH+"/Prancing_Pony/archway", 1);
    }
    else
    {
	set_direction_desc("leading inside");
