...
create_door()
{
    ...
    set_long_bs("A wooden door of thick dark brown boards.\n");
...
door_move(object ob, object from, object to)
{
    ...
    if (to->query_room_file() != BREE_PATH+"/Prancing_Pony/bedroom")
    {
	set_direction_desc("leading into a dark room");
	set_connection(BREE_PATH+"/Prancing_Pony/bedroom", 1);
    }
    else
    {
