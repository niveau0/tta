...
poison_effect(object tp)
{
    ...
    tp->catch_tell("You know something is wrong! Well, you can't breath !\n");
    tell_objects(LISTEN(E(tp)) - ({ tp }),
		({ QCTNAME(tp), "'s face turns blue. After a few seconds "+
		   (tp)->query_possessive()+" face color"+
		   " turns normal again.\n" }),
		   MSG_SEE);
    tp->check_death(3 + random(28), this_object());
    remove_poison(tp);
}
...
query_desc()
{
    return "Uraana, will stop you breathing for some time";
}
