...
test_success(object tp, int clev, int knownlev, int type, int prep)
{
    ...
    /* men-realm casters get penalty for worn head protection */
    if (sz = sizeof(obj = tp->query_worn(SLT_HEAD+1)))
    {
	int mat, metalweight, otherweight;
	...
	for (i=0; i<sz; i++)
	{
		...
		otherweight += obj[i]->query_weight();
	}
	fail += metalweight / 1000 + otherweight / 2000;
    }
    if (roll_dice(-1) < fail)
    {
...
resist(object tp, object *targets, int level, int type)
{
    ...
    {
	if (targets[i] == tp ||
	    targets[i]->spell_resistance(RR_ESS, level, roll, rr_mod) < 1)
